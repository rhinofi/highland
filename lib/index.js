...
/**
 * Same as [ratelimit](#ratelimit) but keeps track of timeouts it
 * starts when rate limiting kicks in and cleans them up when the
 * returned stream is destroyed. Use this if you don't want to wait
 * for events which have already been scheduled whenever the stream
 * is destroyed.
 *
 * @id ratelimitWithCleanup
 * @section Transforms
 * @name Stream.ratelimitWithCleanup(num, ms)
 * @param {Number} num - the number of operations to perform per window
 * @param {Number} ms - the window of time to limit the operations in (in ms)
 * @api public
 *
...
addMethod('ratelimitWithCleanup', function (num, ms) {
    if (num < 1) {
        throw new Error('Invalid number of operations per ms: ' + num);
    }
    var sent = 0;
    const pendingTimemouts = new Set();

    const ratelimitedStream = this.consume(function (err, x, push, next) {
        if (err) {
            push(err);
            next();
        }
            ...
            if (sent < num) {
                ...
                const thisTimeout = setTimeout(function () {
                    pendingTimemouts.delete(thisTimeout)
                    sent = 1;
                    push(null, x);
                    next();
                }, ms);

                pendingTimemouts.add(thisTimeout);
            }
        }
    });

    ratelimitedStream.onDestroy(() => {
        pendingTimemouts.forEach(clearTimeout)
        pendingTimemouts.clear()
    });
...
addMethod('throttle', function (ms) {
    ...
    var resetTimeout = function () {
        clearTimeout(timeout)
        timeout = undefined
        recentSkipped = undefined
    };

    const throttledStream = this.consume(function (err, x, push, next) {
        ...
        if (err) {
            ...
            if (timeout === undefined) {
                var pushRecentAfterMs = ms - (now - last)
                // Schedule push at the edge of throttling interval.
                // This way the most recent value is always eventually
                // emitted instead of being lost.
                timeout = setTimeout(
                    () => {
                        push(null, recentSkipped);
                    },
                    pushRecentAfterMs
                );
            }
    ...
    throttledStream.onDestroy(() => {
        console.log('throttledStream onDestroy')
        resetTimeout();
    });
